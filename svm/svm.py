...
class SVM:
    ...
    def fit_gd(self, X, Y, verbose=False):
        ...
        for e in range(1, self._n_epochs+1):
            for j in range(n_samples):
                ...
                """
                t += 1
                age = 1 / (t * self._lambda)
                score = np.dot(self._w, X[j])

                if Y[j] * score < 1:
                    self._w = (1 - age * self._lambda) * self._w + age * Y[j] * X[j]
                else:
                    self._w = (1 - age * self._lambda) * self._w
            pass
    ...
    def predict(self, X):
        ...
        return self.map_y_to_original_values(predictions)
